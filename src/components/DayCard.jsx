...
export default function DayCard({ week, dayIndex, profile, logs, onClick }) {
  ...
  return (
    <Tag
      ...
      onClick={onClick}
    >
      <span className="dc-num">#{sessionNumber(week, dayIndex)}</span>
      ...
      <span className="dc-top">{s.test ? '1RM test' : `Work to ${s.topRM}RM`}</span>
      <span className="dc-meta">{s.t2Count} T2 · {s.t3Count} T3</span>
    </Tag>
  )
}
